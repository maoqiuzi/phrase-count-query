...
final class PhraseCountScorer extends Scorer {
    ...
    private final CustomPhraseQueue pq; // for advancing min position
    ...
    private CustomPhrasePositions[][] rptGroups; // in each group are PPs that repeats each other (i.e. same term), sorted by (query) offset
    private CustomPhrasePositions[] rptStack; // temporary stack for switching colliding repeating pps
...
//        this.docScorer = docScorer;
        ...
        customPhrasePositions = new CustomPhrasePositions[postings.length];
        for (int i = 0; i < postings.length; ++i) {
            iterators[i] = postings[i].postings;
            customPhrasePositions[i] = new CustomPhrasePositions(postings[i].postings, postings[i].position, i, postings[i].terms);
        }
     ...
     * encountered while traversing/hopping the CustomPhrasePositions.
    ...
    private float phraseFreq() throws IOException {
        ...
        float freq = 0.0f;
        numMatches = 0;
        CustomPhrasePositions pp = pq.pop();
    ...
    private boolean advanceRpts(CustomPhrasePositions pp) throws IOException {
        ...
        CustomPhrasePositions[] rg = rptGroups[pp.rptGroup];
        ...
        while (bits.cardinality() > 0) {
            CustomPhrasePositions pp2 = pq.pop();
            rptStack[n++] = pp2;
            if (pp2.rptGroup >= 0
    ...
    /**
     * Initialize CustomPhrasePositions in place.
    ...
    private void initSimple() throws IOException {
        ...
        for (CustomPhrasePositions pp : customPhrasePositions) {
            pp.firstPosition();
            if (pp.position > end) {
                end = pp.position;
            }
    ...
    private void fillQueue() {
        pq.clear();
        for (CustomPhrasePositions pp : customPhrasePositions) {  // iterate cyclic list: done once handled max
            if (pp.position > end) {
                end = pp.position;
            }
            pq.add(pp);
    ...
    private boolean advanceRepeatGroups() throws IOException {
        for (CustomPhrasePositions[] rg: rptGroups) {
            if (hasMultiTermRpts) {
                ...
                for (int i=0; i<rg.length; i+=incr) {
                    ...
                    int k;
                    while((k=collide(pp)) >= 0) {
                        CustomPhrasePositions pp2 = lesser(pp, rg[k]);
    ...
    private boolean initFirstTime() throws IOException {
        ...
        if (hasRpts) {
            rptStack = new CustomPhrasePositions[numPostings]; // needed with repetitions
            ArrayList<ArrayList<CustomPhrasePositions>> rgs = gatherRptGroups(rptTerms);
            sortRptGroups(rgs);
            if (!advanceRepeatGroups()) {
                return false; // PPs exhausted
            }
    ...
    private void sortRptGroups(ArrayList<ArrayList<CustomPhrasePositions>> rgs) {
        rptGroups = new CustomPhrasePositions[rgs.size()][];
    ...
    private ArrayList<ArrayList<CustomPhrasePositions>> gatherRptGroups(LinkedHashMap<Term,Integer> rptTerms) throws IOException {
        CustomPhrasePositions[] rpp = repeatingPPs(rptTerms);
        ArrayList<ArrayList<CustomPhrasePositions>> res = new ArrayList<>();
        if (!hasMultiTermRpts) {
            ...
            for (int i=0; i<rpp.length; i++) {
                ...
                for (int j=i+1; j<rpp.length; j++) {
                    ...
                    if (g < 0) {
                        g = res.size();
                        pp.rptGroup = g;
                        ArrayList<CustomPhrasePositions> rl = new ArrayList<>(2);
                        rl.add(pp);
                        res.add(rl);
                    }
                    pp2.rptGroup = g;
            ...
            ArrayList<HashSet<CustomPhrasePositions>> tmp = new ArrayList<>();
            ...
            for (int i=0; i<distinctGroupIDs.size(); i++) {
                tmp.add(new HashSet<CustomPhrasePositions>());
            }
            ...
            for (HashSet<CustomPhrasePositions> hs : tmp) {
    ...
    private CustomPhrasePositions[] repeatingPPs(HashMap<Term,Integer> rptTerms) {
        ArrayList<CustomPhrasePositions> rp = new ArrayList<>();
        for (CustomPhrasePositions pp : customPhrasePositions) {
            for (Term t : pp.terms) {
                if (rptTerms.containsKey(t)) {
                    rp.add(pp);
    ...
    public TwoPhaseIterator twoPhaseIterator() {
        return new TwoPhaseIterator(conjunction) {
            ...
            public String toString() {
                return "PhraseCountScorer@asTwoPhaseIterator(" + PhraseCountScorer.this + ")";
            }
        };
    }
